...
> **Skills:** `python-conventions` (style, layer rules, testing) · `ddd-architecture` (patterns, data flows)
...
- **DI:** FastAPI `Depends()` in `api/dependencies.py` provides `Settings`, `ChatEngine`, `IndexingPorts`, and `DocumentIndexingPorts`
- **Providers:** Three LLM/embedding providers (OpenAI, Ollama, HuggingFace). Auto-selection in `infrastructure/providers/selector.py`
...
- `shared/config.py` — All settings with validators, `get_settings()` lazy singleton
