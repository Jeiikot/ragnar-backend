...
| Application | `application/` | domain | api, infrastructure |
...
- **Ports:** `typing.Protocol` in `domain/indexing/ports.py` — never use `abc.ABC`
...
- **DI:** FastAPI `Depends()` in `api/dependencies.py` provides `Settings` and `ChatEngine` as lazy singletons
...
- `domain/indexing/ports.py` — Domain port protocols
